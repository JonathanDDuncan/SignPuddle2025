using System.Xml.Serialization;

namespace SignPuddle.API.Models
{
    /// <summary>
    /// SPML Document class that fully complies with spml_1.6.dtd
    /// DTD: <!ELEMENT spml (term*,text*,png?,svg?,src*,entry*)>
    /// </summary>
    [XmlRoot("spml")]
    public class SpmlDocument
    {
        // Attributes as defined in DTD
        [XmlAttribute("root")]
        public string Root { get; set; } = string.Empty;

        [XmlAttribute("type")]
        public string Type { get; set; } = string.Empty;

        [XmlAttribute("puddle")]
        public int PuddleId { get; set; }

        [XmlAttribute("uuid")]
        public string? Uuid { get; set; }

        [XmlAttribute("cdt")]
        public long CreatedTimestamp { get; set; }

        [XmlAttribute("mdt")]
        public long ModifiedTimestamp { get; set; }

        [XmlAttribute("nextid")]
        public int NextId { get; set; }

        // Elements in exact DTD order: term*,text*,png?,svg?,src*,entry*
        [XmlElement("term")]
        public List<string> Terms { get; set; } = new List<string>();

        [XmlElement("text")]
        public List<string> Texts { get; set; } = new List<string>();

        [XmlElement("png")]
        public string? PngData { get; set; }

        [XmlElement("svg")]
        public string? SvgData { get; set; }

        [XmlElement("src")]
        public List<string> Sources { get; set; } = new List<string>();

        [XmlElement("entry")]
        public List<SpmlEntry> Entries { get; set; } = new List<SpmlEntry>();

        // Helper properties to convert timestamps
        public DateTime Created => DateTimeOffset.FromUnixTimeSeconds(CreatedTimestamp).UtcDateTime;
        public DateTime Modified => DateTimeOffset.FromUnixTimeSeconds(ModifiedTimestamp).UtcDateTime;

        // Helper property for dictionary name (first term)
        public string? DictionaryName => Terms.FirstOrDefault();
    }

    /// <summary>
    /// SPML Entry class that fully complies with spml_1.6.dtd
    /// DTD: <!ELEMENT entry (term*,text*,png?,svg?,video?,src*)>
    /// </summary>
    public class SpmlEntry
    {
        // Attributes as defined in DTD
        [XmlAttribute("id")]
        public int Id { get; set; }

        [XmlAttribute("uuid")]
        public string? Uuid { get; set; }

        [XmlAttribute("prev")]
        public string? Previous { get; set; }

        [XmlAttribute("next")]
        public string? Next { get; set; }

        [XmlAttribute("cdt")]
        public long CreatedTimestamp { get; set; }

        [XmlAttribute("mdt")]
        public long ModifiedTimestamp { get; set; }

        [XmlAttribute("usr")]
        public string User { get; set; } = string.Empty;

        // Elements in exact DTD order: term*,text*,png?,svg?,video?,src*
        [XmlElement("term")]
        public List<string> Terms { get; set; } = new List<string>();

        [XmlElement("text")]
        public List<string> Texts { get; set; } = new List<string>();

        [XmlElement("png")]
        public string? PngData { get; set; }

        [XmlElement("svg")]
        public string? SvgData { get; set; }

        [XmlElement("video")]
        public string? Video { get; set; }

        [XmlElement("src")]
        public List<string> Sources { get; set; } = new List<string>();

        // Helper properties
        public DateTime Created => DateTimeOffset.FromUnixTimeSeconds(CreatedTimestamp).UtcDateTime;
        public DateTime Modified => DateTimeOffset.FromUnixTimeSeconds(ModifiedTimestamp).UtcDateTime;

        // Extract FSW notation (first term is typically the FSW)
        public string? FswNotation => Terms.FirstOrDefault(t => t.StartsWith("AS") || t.StartsWith("M"));

        // Extract gloss (second term is typically the text translation)
        public string? Gloss => Terms.Skip(1).FirstOrDefault();

        // Helper property for first text element (for backward compatibility)
        public string? Text => Texts.FirstOrDefault();

        // Helper property for first source element (for backward compatibility)
        public string? Source => Sources.FirstOrDefault();
    }
}
